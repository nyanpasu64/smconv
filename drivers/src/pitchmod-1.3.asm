...
; (c) 2014? Additional code added by KungFuFurby for pitch modulation and noise generation
;
;========================================================
...
mod_special:	.block 1	;
;-------------------------------;
SM_ACTIVE	=1		; 0: (inactive/active)
SM_PATTERNJUMP	=2		; 1: (off/on)
...
ch_env_tick:	.block 8
ch_env_vib:	.block 8
ch_vib_wav:	.block 8 ; vibrato waveforms
ch_fadeout:	.block 8
ch_ad:		.block 8
ch_sr:		.block 8
ch_end:
...
_not_end:
	...
	and	mod_special, #~SM_PATTERNJUMP
	mov	mod_tick, #0
	mov	mod_row, #0
	ret
...
Module_Update:
;********************************************************
	bbc0	mod_special, _notick	; test SM_ACTIVE
...
;---------------------------------------;
	...
	mov	mod_tick, #0		;
					;
	bbc1	mod_special, _no_pattjump	; test SM_PATTERNJUMP
					;
...
_gain_not_zero:					;
	...
	bne	_gain_not_max			; ...127 is not reached
...
;=======================================================================
	...
	bra	_pt_en				; enable pattern jump
;=======================================================================
...
_pt_en:	or	mod_special, #SM_PATTERNJUMP	;
	ret
;=======================================================================
...
_set_ad:
	mov	ch_ad+x, a			;    M7f: direct gain [default]
	ret					; M80-ff: sets AD values for current channel

;=======================================================================
Command_ChannelVolumeSlide:			; Nxy
;=======================================================================
...
CmdExTab_L:
	...
	.byte	LBYTE(SCommand_VibratoXWav)	; S3x
...
; S01 = turn on echo		S05 = turn on pitch	S09 = turn on noise
; S02 = turn off echo		S06 = turn on picth	S0A = turn off noise
; S03 = turn on echo for all	S07 = turn on pitch all	S0B = turn on noise all
; S04 = turn off echo for all	S08 = turn off pitch "	S0C = turn off noise all
;=======================================================================
SCommand_EPN:	; Echo / Pitchmod / Noise ; KFF added noise & pitch modulation
;=======================================================================
	cmp	a, #09h			; do we need to do something with noise?
	bcc	pitch_mod
...
_sce_disable_all:
	...
	ret
;=======================================================================
SCommand_NoiseFreq2:	; This part added by KFF for noise generation
;=======================================================================
	clrc
	adc	a, #16
;=======================================================================
SCommand_NoiseFreq1:	; This part added by KFF for noise generation
;=======================================================================
	mov	SPC_DSPA, #DSP_FLG
...
;=======================================================================
	bne	cmd_exit3
	cmp	a, #1
	bne	SCommand_Mute			; mute for pitch modulation
	mov	a, ch_flags+x
	or	a, #CF_SURROUND
	mov	ch_flags+x, a
	mov	a, #32
	...
	ret
;=======================================================================
SCommand_Mute:	; This part added by KFF for pitch modulation
;=======================================================================
	cmp	a, #2
	bne	unmute
	mov	a, ch_flags+x
...
;=======================================================================
	...
	bcs	_txxu1
	mov	a, #80
_txxu1:	;cmp	a, #200				; why limit to 200??
	;bcc	_txxu2
	;mov	a, #200
...
;-------------------------------------------------------------------------
	...
	mov	stream_region, a	;
;-------------------------------------------------------------------------
	mov	!StreamAddress+1, a	; copy stream buffer address
