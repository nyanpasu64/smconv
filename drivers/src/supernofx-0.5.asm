...
;-----------------------------------------------------------------------------
	...
	bbc0	special, main_loop		; test SF_MODACTIVE
...
;---------------------------------------;
	...
	bne	_skip_channel		; if not, keep filter sweep enabled
;---------------------------------------;
_reset_fv:
;---------------------------------------;
...
;=============================================================================
	...
	bbc2	special, WTIncrease	; test SF_WTDIR
;---------------------------------------;
	dec	y			;
...
;---------------------------------------;---------------------------
	...
	.word	CMD_LOADWT		; 01h - load wavetable
...
;---------------------------------------;
	...
	bbc5	special, _no_change	; test SM_SWINGTEMPO
;---------------------------------------;
_swing_tempo:
;---------------------------------------;
	mov	a, mod_speed_bk		;
	bbs6	special, _swing_even	; test SF_SWINGODD
;---------------------------------------;
_swing_odd:
;---------------------------------------;
	setc				;
...
Command_Vibrato:
;=======================================================================
	and	special_mode, #~SM_TREMOLO|~SM_PANBRELLO
;-----------------------------------------------
...
_copy_special_coef:
	mov	a, !FILTER+y
	bbc0	special_mode, _csc			; test SM_FILTERSWEEP
...
;------------------------------------------------------------------------
	...
	ret
;------------------------------------------------------------------------
_command_mutechannel:				; S92
;------------------------------------------------------------------------
	mov	a, ch_flags+x
	or	a, #CF_MUTE
	bra	_setcf
;------------------------------------------------------------------------
_command_unmutechannel:				; S93
;------------------------------------------------------------------------
	mov	a, ch_flags+x
...
_s4xit:	and	special_mode, #~SM_NOISEMODE
	ret
;-----------------------------------------------------------------------
_enable_noiserepeat:	;s4c
;-----------------------------------------------------------------------
	or	special_mode, #SM_NOISEREPEAT
	ret
;-----------------------------------------------------------------------
_disable_noiserepeat:	;s4d
;-----------------------------------------------------------------------
	and	special_mode, #~SM_NOISEREPEAT
	ret
;-----------------------------------------------------------------------
_enable_noisepingpong:	;s4e
;-----------------------------------------------------------------------
	or	special_mode, #SM_NOISEMODE
	ret
;-----------------------------------------------------------------------
_disable_noisepingpong:	;s4f
;-----------------------------------------------------------------------
	and	special_mode, #~SM_NOISEMODE
	ret

;=======================================================================
EnableSwingTempo:	;s46
;=======================================================================
