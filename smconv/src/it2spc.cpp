...
#include "math.h"
#ifdef BOOST
#include <boost/algorithm/string.hpp> // for i18n
#else
#include <algorithm>
#endif
...
#include "../drivers/ab-fm-0.1.h"	// spc_ab_fm_program
#include "../drivers/ab-mes-0.1.h"	// spc_ab_mes_program
#include "../drivers/ab-sb-0.4.h"	// spc_ab_sb_program
#include "../drivers/ab-spc-0.2.h"	// spc_ab_spc_program
#include "../drivers/abs-0.1.h"
					// celes-0.1 == ab-fm-0.2
#include "../drivers/celes-0.1.h"	// spc_celes_program
#include "../drivers/fm-rom-0.1.h"	// spc_fm_rom_program
#include "../drivers/pitchmod-1.3.h"	// spc_pitchmod_program
#include "../drivers/snesmod-1.3.h"	// spc_sm_program
#include "../drivers/supernofx-0.5.h"	// spc_supernofx_program
...
int spc_program_size = sizeof ( spc_sm_program );

std::string nl_space = "\n                  ";
std::string author = " Author";
std::string author_ab = "Augustus Blackheart";
std::string author_kff = "KungFuFurby";
std::string author_name = "Mukunda";
std::string author_pt = "psychopathicteen";
std::string authors = "Authors";
std::string authors2 = author_name + nl_space + author_kff;
std::string authors3 = author_name + nl_space + author_ab + nl_space + author_kff;
std::string authors4 = author_name + nl_space + author_ab + nl_space + author_pt + nl_space + author_kff;
std::string driver_name = "SNESmod";
	...
	********************************************************************/
	...
	Driver::Driver ( std::string &driver ) {

		if ( driver == "abfm" || driver == "ABFM" ) {

			author = authors;
			author_name = authors4;
			driver_base = 0x380;
			driver_name = "ABFM";
			patch_byte = 0x2e;
			module_base = 0x1b08;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_ab_fm_program;
			spc_program_size = sizeof ( spc_ab_fm_program );

		} else if ( driver == "abmes" || driver == "ABMES" ) {

			author = authors;
			author_name = authors3;
			driver_base = 0x380;
			driver_name = "AB MVOL EVOL Switch";
			module_base = 0x19c7;
			patch_byte = 0x2b;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_ab_mes_program;
			spc_program_size = sizeof ( spc_ab_mes_program );

		} else if ( driver == "abnofx" || driver == "ABNOFX" ) {
			...
			module_base = 0x19ae;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_ab_sb_program;
			spc_program_size = sizeof ( spc_ab_sb_program );

		} else if ( driver == "abstream" || driver == "ABSTREAM" ) {

			author = authors;
			author_name = authors3;
			driver_name = "ABSTREAM";
			module_base = 0x1ac6;
			spc_program = (unsigned char*)spc_abs_program;
			spc_program_size = sizeof ( spc_abs_program );

		} else if ( driver == "abspc" || driver == "ABSPC" ) {

			author = authors;
			author_name = authors4;
			driver_base = 0x380;
			driver_name = "ABNoFX SPC";
			patch_byte = 0x00; //skip patch routine
			module_base = 0x1916;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_ab_spc_program;
			spc_program_size = sizeof ( spc_ab_spc_program );

		} else if ( driver == "fmrom" || driver == "FMROM" ) {

			author = authors;
			author_name = authors4;
			driver_base = 0x380;
			driver_name = "ABNoFX demo ROM";
			module_base = 0x1a53;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_fm_sb_program;
			spc_program_size = sizeof ( spc_fm_sb_program );

		} else if ( driver == "celes" || driver == "CELES" ) {
			...
			module_base = 0x1afa;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_celes_program;
			spc_program_size = sizeof ( spc_celes_program );

		} else if ( driver == "pitchmod" || driver == "PITCHMOD" ) {

			author = authors;
			author_name = authors2;
			driver_name = "Pitchmod";
			module_base = 0x1900;
			spc_program = (unsigned char*)spc_pitchmod_program;
			spc_program_size = sizeof ( spc_pitchmod_program );

		} else if ( driver == "supernofx" || driver == "SUPERNOFX" ) {

			author = authors;
			author_name = authors3;
			driver_base = 0x380;
			driver_name = "Super " + driver_name + " NoFX";
			patch_byte = 0x2b;
			module_base = 0x1aa0;
			sample_table_offset = 0x180;
			spc_program = (unsigned char*)spc_supernofx_program;
			spc_program_size = sizeof ( spc_supernofx_program );

		}

		header_size = 32 + max_length + max_patterns*2 + max_instruments*2 + max_samples*2;
	...
	Info::Info() {
		...
		if ( INFO ) {
			printf( "         %s: %s\n"
				"     Driver base: 0x%x\n"
				"     Module base: 0x%x\n"
				, author.c_str(), author_name.c_str(),
				driver_base,
				module_base
				);
		}
...
//		EchoEnable = cinput.EON;
		...
		int envsize = 0;
                for (u32 i = 0; i < Instruments.size(); i++ ) {
			envsize += Instruments[i]->GetEnvSize();
		}

		u32 echosize = EchoDelay * 2048;
		u32 totalsize = pattsize + sampsize + instrsize + envsize + echosize + header_size+(mod.SampleCount*2)+4;
		spc_ram_size = 65535 - module_base;
		u32 bytesfree = spc_ram_size - totalsize;
		...
		if( VERBOSE ) {
			printf( 
				...
				header_size+(mod.SampleCount*2)+4,
				echosize,
				totalsize, bytesfree
				);
				Line();
			if( totalsize > spc_ram_size ) {
				printf( "      MODULE IS TOO BIG. maximum is %5i bytes\n", spc_ram_size );
			}
		}
	}
	...
	static int SearchForSNESMODtag( const char *message ) {
		...
		const char *lc_test = "[[snesmod]]"; //don't make me type uppercase
		int matches = 0;

		for( int i = 0; message[i]; i++ ) {
			if( message[i] == test[matches] || message[i] == lc_test[matches] )
				matches++;
			else
				matches = 0;
			if( matches == 11 )
...
//			nstr += ".spc";
		...
		//	IO::File file( nstr.c_str(), IO::MODE_WRITE );
			...
			for ( int i = 0; i < spc_program_size; i++ ) {
				if( patch_byte != 0 && ( i == patch_byte || i == patch_byte+1 )) // PATCH
					file.Write8( 0 );
				else
	...
	void Bank::Export( const char *output ) const {
		...
		for( u32 i = 0; i < max_patterns*2 + max_instruments*2 + max_samples*2; i++ )
			file.Write8( 0xAA );
		...
		std::string soundbank = output;
		#ifdef BOOST
		boost::to_upper(soundbank);
		#else
		std::transform( soundbank.begin(), soundbank.end(), soundbank.begin(), (int(*)(int))std::toupper );
		#endif
		std::string asm_out = output;
		asm_out += ".asm";
		ExportASM( soundbank.c_str(), bin_output.c_str(), asm_out.c_str() );

		std::string inc_out = output;
		inc_out += ".inc";
		ExportINC( soundbank.c_str(), inc_out.c_str() );
	}

	void Bank::ExportINC( const char *output, const char *inc_out ) const {
		FILE *f = fopen( inc_out, "w" );

		fprintf( f,
			"; snesmod soundbank definitions\n\n"
			".ifndef __%s_DEFINITIONS__\n"
			".define __%s_DEFINITIONS__\n\n"
			".import __%s__\n\n",
			output,
			output,
			output
		...
		fprintf( f, "\n.endif ; __%s_DEFINITIONS__\n", output );
		fclose(f);
	}

	void Bank::ExportASM( const char *output, const char *inputfile, const char *outputfile ) const {
		...
		fprintf( f, 
			...
			"\n"
			"\t.global __%s__\n"
			"\t.segment \"%s\" ; need dedicated bank(s)\n\n"
			"__%s__:\n",
			size,
			output,
			output,
