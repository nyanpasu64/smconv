...
namespace IT2SPC {
	...
	public:
		...
		int GetExportSize() const {
			//return 5 + (!EnvelopeLength) ? 0 : (3 + (EnvelopeLength/4) * 4); // this always returns 0
			return 5 + (!EnvelopeLength); // instr size only
		}

		int	GetEnvSize() const {
			return (3 + (EnvelopeLength/4) * 4);
		}
	...
	public:
		...
		void ExportASM( const char *, const char *, const char * ) const;
		void ExportINC( const char *, const char * ) const;
